...
return {
	...
	["it should fail if a throwing function is expected to never throw"] = function()
		...
		assert(message:match("Expected function to succeed, but it threw an error:"), ("Error message does not match:\n%s\n"):format(message))
	end,
	["it should fail if an empty function is expected to throw"] = function()
		...
		assert(not success, "should fail")
		assert(message:match("Expected function to throw an error, but it did not."), ("Error message does not match:\n%s\n"):format(message))
	end,
	...
	["it should fail if a throwing function is expected to throw with a different error message"] = function()
		...
		assert(message:match('Expected function to throw an error containing "foo"'), ("Error message does not match:\n%s\n"):format(message))
		-- Make sure we state the actual error
		assert(message:match(": oof"), ("Error message does not match:\n%s\n"):format(message))
	end,
	...
	["it should fail if a throwing function is expected to never throw a substring of the message"] = function()
		...
		assert(not success, "should fail")
		-- Make sure we state the expected error
		assert(message:match('Expected function to never throw an error containing "oof"'), ("Error message does not match:\n%s\n"):format(message))
		-- Make sure we state the actual error
		assert(message:match(": foo%-oof"), ("Error message does not match:\n%s\n"):format(message))
	end,
	...
	["nil should be not ok"] = function()
		...
		assert(message:match('Expected value "nil" to be non%-nil'), ("Error message does not match:\n%s\n"):format(message))
	end,
	["false should be ok"] = function()
		...
		assert(message:match('Expected value "false" to be nil'), ("Error message does not match:\n%s\n"):format(message))
	end,
	...
	["different values should not be equal"] = function()
		...
		assert(not success, "should fail")
		assert(message:match('Expected value "5" %(number%), got "5" %(string%) instead'), ("Error message does not match:\n%s\n"):format(message))
	end,
